...
ig.global.EntityDialog = ig.Entity.extend({
    ...
    init: function(x, y, settings) {
        ...
        this.text = ig.game.controller.splitLines(this.text,this.dialogFont, this.size.x + 60);
        if (this.dialogType === 'thanks') {
            if (ig.game.currentLevel === LevelGameover) {
                this.size.y = 110;

                ig.game.spawnEntity(EntityResponse, this.pos.x + this.size.x + 5, this.pos.y, {parentEntity: this, mood: null});
            }
            else {
